...
if __name__ == '__main__':
    ...
    dataset_name = "TUEV"
    number_of_samples_per_class = 32
    ...
    batch_size = number_of_samples_per_class * n_classes // 4
    ...
    if dataset_name == "TUAB":
        ...
        # pick only a few samples per class
        train_dataset = TUABLoader(os.path.join(dataset_root, "train"), train_files, sampling_rate)
        train_dataset.select_samples(number_of_samples_per_class)
    ...
    log_dir = os.path.join(model_name, dataset_name, f"{number_of_samples_per_class}-samples-per-class", time.strftime("%Y-%m-%d-%H-%M-%S") + f"_{number_of_samples_per_class}-samples-per-class")
